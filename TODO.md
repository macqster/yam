...
- `refactor` prune the per-item `#[allow(dead_code)]` set in `src/`. Module-wide `#![allow(dead_code)]` is gone as of 2026-09-06, but 127 per-item allows remain and removing all of them leaves only 67 dead-code warnings, each covering several items - so a good share guard items that have real callers and are simply stale. Measure with `#[expect(dead_code)]` plus `RUSTFLAGS="--warn=unfulfilled_lint_expectations" cargo check --all-targets`, which names every allow that is doing nothing; keep the ones covering genuinely forward-built surface, per `docs/hygiene.md`.
- `verify` follow-hero camera mode stays centered on the world datum across terminal resizes, while manual pan mode remains clamped to world overscan - and that both clamp the *rendered* camera (`camera.x`/`camera.y`) only, never the authored `offsets.camera_x`/`camera_y` that get persisted (2026-09-06); per-frame paths must not author state.
