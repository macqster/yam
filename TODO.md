...
- [inspect] continue the render-loop efficiency pass after the widened per-layer scratch-grid slice: take the heavier always-active layers next (`hero`, `debug`, possibly `vines`) and only then revisit whether any diagnostics should move to even cheaper draw paths.
