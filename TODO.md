...
- [inspect] continue the render-loop efficiency pass after the hero scratch-grid slice: take the remaining heavier always-active layers next (`debug`, possibly `vines`) and only then revisit whether any diagnostics should move to even cheaper draw paths.
