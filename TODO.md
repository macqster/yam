...
- [inspect] continue the render-loop efficiency pass after the first `Scene` reuse and hidden-layer skip slices: investigate reusable `Grid` scratch/final buffers, and decide whether some always-closed-on-main-scene diagnostics should move to even cheaper draw paths.
