...
- [inspect] continue the render-loop efficiency pass after the first per-layer scratch-grid slice: widen scratch-grid adoption to heavier always-active layers (`hero`, `clock`, `weather`, `date`, `debug`, possibly `vines`) and only then revisit whether any diagnostics should move to even cheaper draw paths.
