...
- [inspect] continue the render-loop efficiency pass after the first final-buffer reuse slice: investigate reusable per-layer `Grid` scratch buffers, and decide whether some always-closed-on-main-scene diagnostics should move to even cheaper draw paths.
