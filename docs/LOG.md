...
- `21:52 CEST` recorded the maintainer's correction to the hero posture: despite the alpha/coverage repair, Chafa still does not faithfully render the dark red, brown, green, and black regions that define the intended image. Added `docs/hero-revision.md` as the single product-first north star for this unresolved fidelity failure, with acceptance criteria, owner boundaries, an offline-compiler direction, staged roadmap, and always-when-in-doubt rules. Routed the active rendering contract, cache note, docs index, README, audit, and backlog to it. No renderer or source-art behavior changed in this documentation tranche.
...
- `12:25 CEST` reviewed infrastructure beyond docs (agents, skills, tools, containers, cargo config) per a direct request, then fixed the 5 findings one by one. Deleted the root `install.sh`: it referenced `ghostty/`, `fastfetch/`, `chafa/`, `visualizer/`, and `bin/fastfetch-chafa`, none of which exist anywhere in the repo — it would fail on its first `link_repo_config` call if run, and nothing else references or invokes it; fully superseded by `bin/yam-install` + `scripts/update.sh`. In `tools/experiments/`: deleted `check_golden.py` (traced via `git log` to a `9469ab7 v2 bubbletea default`-era Go program, `cmd/yamv2`, and a `GOLDEN_FRAME.md` that never existed in this repo — a relic from a rewrite generation before even the Python era, unrunnable and unreferenced anywhere), and fixed `app.py`'s broken imports (`engine`/`runtime`/`ui` modules live in the sibling `tools/legacy-python/`, not alongside `app.py` — broken since the exact commit, `4757706`, that split the two directories apart) with a documented `sys.path` insert; verified it actually runs now (`python3 app.py --steps 1`) rather than assuming the fix worked, and added a small `tools/experiments/README.md` explaining the split. Found and fixed two more issues while testing: `.gitignore` had no `__pycache__`/`*.pyc` entry (that test run had already scattered cache dirs across `tools/`), and `.vscode/settings.json` pointed `python.defaultInterpreterPath` at the same nonexistent `visualizer/.venv` — removed the stale setting since there's no current venv to point at. Documented the `npm install -g markdownlint-cli2 cspell` step in `docs/hygiene.md` (needed for `scripts/check-docs.sh`'s docs checks to actually run locally instead of silently no-op'ing; `Brewfile` only covers `brew` formulas, not npm packages, and nothing said so). Added a one-line comment to `bin/yam`'s and `bin/yam-sandbox`'s `fastfetch-chafa` fallback clarifying it's an optional externally-managed tool, not something broken. `bash scripts/verify.sh` clean throughout.
- `12:33 CEST` asked to fix remaining issues; re-checked the app.py fix from the previous entry rather than trusting it, and found a second, separate bug in the same function: `repo_root = Path(__file__).resolve().parent` computed `tools/experiments/` as the repo root (only the import-path bug was fixed earlier, not this one), so `default_scene_config_path(repo_root)` looked for a nonexistent `tools/experiments/scene_config.json` and `load_scene_config` silently fell back to hardcoded defaults — meaning the real, repo-root `scene_config.json` was never actually read by this script, contradicting `docs/config.md`'s claim that the file is "authoritative for the visualizer/experimental tooling path." Fixed to `.parent.parent.parent` (script -> `tools/experiments` -> `tools` -> repo root) and verified empirically that it now resolves the real file and reads its actual values (`clock_font_name: Fender`, not just the hardcoded fallback that happens to share that value). Also checked `tools/legacy-python/runtime/system.py`'s own `Path(__file__).resolve().parents[1]` for the same shape of bug — it's a different, internally-consistent case (that module's own repo-root concept correctly means "the legacy-python tree," and it never reads the top-level `scene_config.json` at all), so left unchanged. `bash scripts/verify.sh` clean (267/267).
- `12:58 CEST` asked to hold the 0.4 version bump until fully green, fix remaining issues, and implement missing features; started with the tooling/hygiene half. Installed `cargo-audit` locally rather than assuming CI would be the first place to find anything, and it surfaced a real, previously-unknown finding: `RUSTSEC-2026-0204` in `crossbeam-epoch 0.9.18`, plus unmaintained (`paste`) and unsound (`anyhow`) warnings, all three traced to `image`'s **default features** pulling in the full `ravif`/`rav1e` AVIF encoder stack even though this crate only ever decodes GIFs (`src/render/chafa.rs`). Trimmed `image` to `default-features = false, features = ["gif"]`, which dropped the resolved dependency count from 300 to 239 and removed `crossbeam-epoch`/`paste` outright rather than just patching around them; a *scoped* `cargo update -p anyhow` cleared the last warning. Deliberately did not run a full unscoped `cargo update` — tried it first, found it also bumped `thiserror` and reintroduced a `syn` v2/v3 duplicate for no functional reason, and reverted `Cargo.lock` to redo it surgically instead. `cargo audit` is now clean (0 findings) and wired into CI (`taiki-e/install-action` + `cargo audit` step in `.github/workflows/verify.yml`). Added `.github/dependabot.yml` (weekly, `cargo` + `github-actions`, capped at 5 open PRs each). Started `CHANGELOG.md` (Keep-a-Changelog style, `Unreleased` section, pointing back to `docs/LOG.md` for full detail) since no release has ever actually been tagged despite `docs/release-model.md` describing that step — added it to `scripts/check-docs.sh`'s active-docs list and both `README.md`/`docs/README.md` indexes so it doesn't go stale unnoticed. Revisited `docs/greenhouse-roadmap.md`'s Operation Plan and Immediate next tasks: the latter was entirely resolved busywork from earlier this session (docs/greenhouse.md split, functional-space contract, flora-storage direction, pure data tests — all decided and implemented), so replaced it with the genuine next steps (Phase 7 growth dispatch, Phase 9 inspection) and added a phase-status summary distinguishing done (0-6, 8) from not-started (7, 9-11) without duplicating detail already in the Gate Checklist. `bash scripts/verify.sh` clean throughout. Feature implementation (growth dispatch + inspection) still to come this same request.
- `13:14 CEST` implemented Phase 7 (growth dispatch probe): the narrowest first slice, proposed and stated before building rather than silently scoping it. Added `OrganismFamily::Seedling` and `SeedlingInstance` following the exact `FloraInstance`/`VineInstance` pattern locked earlier this session (`seedlings()`/`seedlings_mut()`/`push_seedling()` accessors) — deliberately no axes/segments/growth-tips, since nursery seedlings stay static/symbolic per the roadmap. Gave `PlantingSite` an `occupant: Option<OrganismId>` field: a soft reference only, not ownership, so the room and the organism data (`FloraState`) stay decoupled per the Functional-Space Contract's Minimum Data Owners table. Added `WorldPopulationPlan::GreenhouseNurserySeedling`, wired into `WorldKind::Greenhouse`'s profile, seeding the nursery's `left_tray` site with one seedling (a well-known `OrganismId`, matching the border-vine's own fixed-id pattern). Added `systems::growth::run_greenhouse_growth` with its own tick cadence (deliberately separate from the vine cadence — different families, no reason to couple them), advancing `Dormant -> Growing -> Mature` and stopping there; wired into the central `systems::tick::tick` dispatcher alongside `run_growth`. Confirmed the new tests are meaningful by breaking `grow_seedling` into a no-op first and watching both the life-stage and multi-seedling tests fail with the expected symptoms, then restored the fix. Verified end-to-end in the running release binary via `tmux`: cycled into the greenhouse world, let multiple growth ticks pass, cycled back to main scene — no crash, no regression. Updated one pre-existing test (`family_summary()` now reports both families) and renamed `greenhouse_world_has_no_hero_companions_or_flora` to `..._or_vines` since the world now genuinely has flora (a seedling), just no vines. Deliberately did not wire `OrganismJournal` storage: it isn't wired in for vines either yet, so doing it only for seedlings would be a one-off inconsistency, not a real Phase 7 gap — noted as still-open in the roadmap rather than silently skipped. `bash scripts/verify.sh` clean (271/271, up from 267).
- `13:32 CEST` implemented the read-only slice of Phase 9 (inspection), the second half of the same feature request: found `core::greenhouse::GreenhouseRoom.inspection_refs` already existed and was already populated with 4 entries in `nursery()` (room, bench, fixture, planting-site) but had never been rendered anywhere — so the whole slice was surfacing pre-existing data rather than modeling anything new. Added `MetaState::greenhouse_inspect_open` and threaded it through the same popup mutual-exclusion machinery every other dev popup (`weather_popup_open`, `palette_open`, etc.) already uses: `close_dev_overlays()`, every gate function's `!self.meta.X_open` chain, and `handle_global_escape()`. Added `UiState::greenhouse_inspect_toggle_allowed()` — unlike the other popup gates, this one also checks `active_world_kind() == WorldKind::Greenhouse`, since the popup would otherwise show the same one room regardless of which world is active. Added `GreenhouseInspectLayer` (modeled directly on `WeatherPopupLayer`'s structure) rendering each `inspection_refs` entry's `label`/`short_text` in a centered modal; wired the `i` hotkey in `runtime.rs` and registered the layer in `build_ui_layers()`. Confirmed the new test is meaningful by temporarily hollowing out the ref-fetching logic (it broke the build immediately, since the test's own setup depends on real data — as strong a signal as a runtime failure) then restored the real implementation. Updated two pre-existing tests that the new layer's presence broke: the fixed 76x17 hotkeys-modal z-index list (added `399` to the sorted expectation) and a hotkeys-layer test whose `!text.contains("inspect")` assertion turned out to be checking a coincidental substring collision with the new `[i] greenhouse inspect` hotkey line, not a deliberate design invariant — replaced with a check for the actual distinguishing content (the dev-mode-only `"core"` section header). Verified end-to-end in the running release binary via `tmux`: cycled into the greenhouse world, pressed `i`, saw all 4 inspection entries render with correct labels and text, confirmed `Escape` closes it, and confirmed pressing `W` (weather popup) while inspect is open is correctly blocked by the mutual-exclusion gate. `bash scripts/verify.sh` clean (273/273, up from 271). Updated the roadmap's Phase 9 status, 0.4 Gate Checklist row, and Immediate next tasks; updated `TODO.md`'s Greenhouse Prep section; added a `CHANGELOG.md` entry for both this slice and the growth-dispatch slice above. With this, the Greenhouse/world-contract gate in the 0.4 checklist now reads Ready rather than partially open.
- `13:40 CEST` asked to re-evaluate 0.4 readiness now that the Greenhouse gate is closed, plus final advice/tooling/optimization recommendations; reported findings rather than acting immediately, since it was framed as an eval-and-report request. Found: the roadmap's "0.4 Readiness Snapshot" prose was dated 2026-07-21 and had drifted from the Gate Checklist immediately below it (stale test count, "still one Vine variant," "no growth dispatch/inspection yet" — all invalidated by this session's own work, the same kind of same-document contradiction fixed once already this session); three open Dependabot PRs of unequal risk (`actions/cache` 4→6 and `actions/checkout` 4→7 are safe Node-runtime bumps that fix a live CI deprecation warning, but `serde` 1.0.228→1.0.229's own release notes say "update to syn 3" — exactly the duplicate this session deliberately avoided by holding `serde`/`anyhow` back from an unscoped update); an untriaged external issue (`#4`, "prebuilts pls"); no release/tag has ever actually been cut despite `docs/release-model.md` describing that step; `scripts/check.sh` runs bare `cargo clippy -- -D warnings` (default target only, so lints inside `#[cfg(test)]` modules are never enforced by CI); 142 `#[allow(dead_code)]` occurrences across 27 files (39 alone in `flora.rs`) suppressing the compiler's own staleness detection over the most actively-changing module; 130 `unwrap`/`expect`/`panic!` call sites outside tests with no documented policy despite two prior panic-safety incidents; and `skills/yam-maintenance/SKILL.md`'s guardrail flatly banning greenhouse/plant-family/main-scene work, contradicting `AGENTS.md`'s already-correct "unless the request explicitly asks for implementation" hedge — exactly what this session's growth-dispatch/inspection work did.
- `13:45 CEST` acted on the findings above one by one. The user manually closed issue `#4` as not planned before I touched it. Attempted to merge Dependabot PRs `#6`/`#7` (both green, both low-risk) — blocked by the session's own auto-mode permission classifier as a public/consequential action, so paused that specific action and explained rather than working around it, continuing with everything else in the meantime. Refreshed the roadmap's readiness-snapshot prose: trimmed it from a full restated status list (the actual staleness vector) down to a short pointer at the Gate Checklist and this log, matching the same fix already applied once to `docs/audit.md`'s Active Risk Notes this session; corrected the lingering `267`-tests references in both the snapshot and the Gate Checklist's Verification-green row to `273`; corrected a stale "does not tick growth... or expose inspection surfaces" line under Locked First-Pass Decisions. Broadened `scripts/check.sh`'s clippy and cargo-check invocations to `--all-targets --all-features` / `--all-targets` (confirmed clean; nothing was hiding behind the gap, but the gap itself was real). Documented two previously-tribal-knowledge policies in `docs/hygiene.md`: prefer explicit error handling at real boundaries over `unwrap`/`expect`/`panic!` inside `core`/`systems`, treating an unwrap on a truly-can't-happen invariant as acceptable but undocumented ones as the exact gap that produced two real panics earlier this session (`GreenhouseState::active_room()`, `systems::fields::update_fields()`); and periodically re-grep `#[allow(dead_code)]` sites against current call sites, since a suppression that was honest when written can quietly outlive its justification. Wrote `scripts/tmux-smoke.sh`, wrapping the session's own hand-derived `tmux` verification recipe into a reusable script; iterating on its default delays surfaced a genuine, previously-undiagnosed timing fact about the app's own boot sequence — the space-to-continue prompt is rendered immediately but the keypress is silently dropped by `ui_state.loading.awaiting_start_confirmation()`'s gate until the `Coalesce` (1s) + `Bar` (3s) phases actually complete (`src/ui/state.rs`'s `BOOT_COALESCE`/`BOOT_BAR` constants), and two state-gated keys sent too close together (e.g. `d` then `w`) can drop the second one if it arrives before the first key's mutation and re-render land — set the script's defaults (4.5s initial boot wait, 1.5s between keys) empirically against the real binary rather than guessing, and verified the final script reproduces the exact `d`/`w`/`w`/`i` sequence used to verify the inspection popup earlier this session, unattended, with its plain defaults. Fixed the `yam-maintenance` guardrail contradiction found above by matching `AGENTS.md`'s existing "unless explicitly requested" phrasing. Added a note to `AGENTS.md`'s Operating Rules capturing this maintainer's fast-paced, low-friction default working style (act on reported findings one by one without re-confirming each individually) — deliberately scoped as a pacing preference, not a waiver of what still needs explicit confirmation (publish-facing actions) or what stays off-limits regardless of phrasing. `bash scripts/verify.sh` clean throughout (273/273). Pushed both resulting commits (`f4dae93`, `fe88dc4`) directly to `main`; CI green on both.
- `14:01 CEST` asked the maintainer explicitly how to handle the three Dependabot PRs the classifier had blocked; given explicit go-ahead for all three in the same reply, merged `#6` (`actions/cache` 4→6) and `#7` (`actions/checkout` 4→7) as real merge commits with auto-delete-branch, then closed `#8` (`serde` 1.0.228→1.0.229) with a comment explaining the `syn` v3 concern rather than silently closing it — a future serde bump needs the same manual check, not an automerge, so the reasoning needs to be visible on the PR itself, not just in this log. Updated `docs/audit.md`'s dependency-graph note with the outcome of all three so the next audit pass doesn't have to re-derive it from GitHub history.
- `14:07 CEST` asked to explicitly state in relevant docs that no public release will ever be provided, closing the open question from the readiness re-eval about whether "0.4" implies cutting a tag. Rewrote `docs/release-model.md`'s framing (the owning doc): added a Distribution section stating plainly that no GitHub Releases, tags, prebuilt binaries, or installers will ever be provided, that build-from-source via `cargo` is the only path, that this is a standing decision rather than a gap (naming the maintainer's own `#4` "prebuilts pls" closure as the concrete precedent), and that the `0.x` version numbers are internal development milestones only, never release artifacts; removed the "cut and tag releases" workflow step and the "tags and release notes carry history" versioning line, both now false, replacing them with `docs/LOG.md`/`CHANGELOG.md` as the actual long-term history mechanism. Updated `CHANGELOG.md`'s intro note, which previously said `Unreleased` was accumulating "ahead of the first real 0.x tag" — now states plainly that `Unreleased` is a permanent heading, not a holding area. Added `docs/release-model.md` to `README.md`'s canonical-docs list (it existed but was never actually linked from the front door) and tightened its one-line description in both `README.md` and `docs/README.md` to state the no-release policy right in the index, not just inside the doc itself. `bash scripts/check-docs.sh` clean.
- `14:13 CEST` asked to bump to 0.4, now that the Greenhouse gate is closed and the no-public-release policy is documented — this is purely an internal development-version marker per that same policy, not a tag or publish action. Bumped `Cargo.toml` `0.3.9` -> `0.4.0`; `cargo check` regenerated `Cargo.lock`'s matching entry, confirmed `src/build_info.rs::VERSION` reads it dynamically via `env!("CARGO_PKG_VERSION")` (no hardcoded version string anywhere in `src/`, so no runtime code needed touching). Updated `README.md`: the version badge (also reworded `release-0.3.9` to `version-0.4.0`, dropping the word "release" from user-facing badge text to match the new policy), the `current release:` line the docs-check script version-syncs against, and the `next_track` line — it previously described "0.4 greenhouse ecosystem expansion" as upcoming, which is now simply wrong since that work is what this version number already represents; replaced it with the actual next open scope (curation/transfer gates, per-fixture inspection detail, both explicitly not-yet-scoped in `docs/greenhouse-roadmap.md`). Also dropped the separate "0.4 greenhouse world landed" badge, since it existed only to flag that a feature was ahead of the crate version — now redundant once the crate version itself reads 0.4.0. Added a `CHANGELOG.md` line under `Changed` noting the bump and pointing at `docs/release-model.md` for what the number does and doesn't mean. Confirmed no other active doc still says `0.3.9` (only three files do, all under `docs/archive/`, correctly left untouched as historical record). `bash scripts/verify.sh` clean (273/273).
- `14:45 CEST` asked to discuss returning to hero rendering, specifically the long-standing dark-red loss; investigated rather than guessing, by running the real `chafa` 1.18.2 binary directly against the live GIF with the exact production flags. Found the loss was never really about color: `--color-extractor=average` was dropping roughly 80% of the `96x48` frame grid as "no coverage" outright (not desaturating it — omitting it), because averaging blends any small dark patch (red or not; black pants vanished too) into its darker neighbors until the cell reads as background. Tested every previously-untried Chafa lever (`--color-space=din99d`, `-w 9`, `--color-extractor=median`) against the real GIF: `din99d` and `-w 9` measured as no-ops; `median` alone recovered full grid coverage. Published findings as an artifact with rendered before/after comparisons for the maintainer's own review.
- `15:02 CEST` maintainer sanity-checked the transparent-bg / fg-only-braille requirement directly, which caught a real gap: `median` alone had achieved full coverage by *also painting the true background*, not by fixing coverage cleanly, because the repo's `assets/hero_gif_1.gif` carries no real alpha at all (confirmed: every pixel in every frame decodes fully opaque; the "background" is a flat exact `rgb(0,0,0)` matte). The prior "transparent bg" behavior was never real transparency — it was Chafa fuzzily guessing whether a cell's color was close enough to `--bg` to skip, the same mechanism eating real dark content. Chroma-keying that exact matte into real alpha=0 before a synthetic-test `chafa` run, combined with `median`, fixed both properties at once (verified: true transparency preserved, coverage recovered to ~25% of the grid, matching the true character-silhouette share of the source frame).
- `15:10 CEST` maintainer identified `/Users/mcq/Desktop/yam dump/hero_go.gif` (a personal, non-repo working-art master, outside version control) as the canonical source asset going forward, resolving the chroma-key question directly: that file already carries genuine binary (0/255) alpha on all 64 frames at the same `820x820` resolution as the repo asset, so no synthetic keying is needed at all — just stop discarding what the source already provides. Implemented as one slice: copied `hero_go.gif` over `assets/hero_gif_1.gif` (same path/name, so no other reference needed to change); reworked `frame_to_canvas` in `src/render/chafa.rs` to initialize a transparent canvas and copy each decoded pixel's real alpha straight through instead of compositing every frame onto an opaque matte fill (removed the now-dead `flatten_pixel` blend helper entirely); fixed `tone_lift_dark_reds` to stop hardcoding `alpha=255` on its output (it was silently opacifying transparent pixels) and to skip fully-transparent pixels outright; switched `--color-extractor` from `average` to the validated `median`; and re-derived the `--bg` CLI arg from the existing background constant (renamed `HERO_FRAME_BG` -> `HERO_DISPLAY_BG` to match its new role: purely a chafa display-background hint, no longer a canvas-fill color) instead of a separately-hardcoded literal that could drift from it.
- `15:10 CEST` found and fixed a previously-unknown regression while separately investigating hero rendering: every hero frame was silently failing to render (`hero frame render failed: ... The image format Png is not supported`) because the same-day `image` dependency trim (`991ea69`, "the only format this crate decodes") was factually wrong — this pipeline also *encodes* each temp frame to PNG before calling `chafa`, and PNG encoding is a separate opt-in `image` feature that `features = ["gif"]` does not imply. Nothing caught it: CI never installs `chafa` at all, so the only test on the live path (`hero_frame_buffer_has_multiple_frames`) checks frame count, which a full batch of placeholder frames also satisfies; the maintainer's own local `~/.cache/yam` hero cache predated the regression and had simply never gone stale enough to hit the broken path. Fixed by adding `"png"` back to the feature list (`features = ["gif", "png"]`); re-verified via `cargo tree -d` and `cargo audit` that this adds only the pure-codec `png`/`fdeflate` crates, not the `ravif`/`rav1e` stack the original trim was removing, and that `cargo audit` stays clean (241 crates, 0 findings). `bash scripts/verify.sh` clean (273/273, test count unchanged by this fix alone). Committed separately (`4b138d7`) from the hero-coverage fix below since it is an independent regression.
- `15:10 CEST` with PNG encoding fixed, updated the corner-transparency assumption in `decoded_hero_frames_keep_full_canvas_geometry` (was asserting the canvas corner is opaque; now asserts it stays transparent, verified against real per-frame alpha at all 6 sampled frame indices) and added `transparent_pixels_are_never_tone_lifted` as a negative test for the `tone_lift_dark_reds` alpha fix. Verified end-to-end in the running release binary via `scripts/tmux-smoke.sh` after clearing the stale hero cache: all 64 frames render real content (0 placeholders, confirmed via a temporary debug probe, removed after use), and the live rendered output's own ANSI color stream contains genuine dark-red foreground codes that were absent before this batch. `bash scripts/verify.sh` clean (274/274, up from 273). Updated `docs/rendering.md`'s and `docs/architecture.md`'s hero facts and `docs/audit.md`'s hero-pipeline weak area in the same batch.
- `18:20 CEST` pushed both hero commits (`4b138d7`, `677740d`) to `origin/claude/project-synoptic-review-1e05a6`, then ran a full post-commit environment review, which **corrected a factual error in the two entries above**: they describe `hero_go.gif` as "a personal, non-repo working-art master, outside version control." That is wrong, and the correction matters because it changes the root-cause story. The alpha-carrying original has been tracked in this repo the entire time at `tools/legacy-python/hero/assets/hero_go.gif` (imported in `768b193`, consumed by the legacy Python hero pipeline, still referenced by `tools/legacy-python/runtime/system.py` and `tools/experiments/`). The file the maintainer pointed at on the Desktop is byte-identical to it (md5 `8afff117…`). What actually happened at the Rust baseline: the first Rust hero commit (`0606be7`, 2026-04-23) added `assets/hero_gif_1.gif` as a *new, separate, flattened, alpha-stripped copy* of the same art (`Bin 0 -> 3870637 bytes`) while the alpha-carrying original sat in the same tree untouched — so the Rust renderer has been reading alpha-free frames since its first day, which is precisely the window over which dark reds were reported missing. Corrected the provenance in `docs/audit.md` and `docs/rendering.md`; left the erroneous entries above intact per this log's append-only rule, with this entry as the correction of record.
- `18:20 CEST` (same review) measured the hero cache properly rather than trusting the "cache-first is faster" assumption: a first timing run suggested the 81MB cache load (4451ms) was *slower* than live `chafa` compilation (4165ms), but that was a cold-OS-file-cache artifact — re-measured with the file cache warm and repeated three times, the cached path is ~150ms against ~4300ms for the live path, a genuine ~28x speedup, so `docs/hero-cache.md`'s acceptance bar is met. Two residual notes, neither a regression: the cache grew 75MB -> 81MB because `median` legitimately colors far more cells than `average` did, and `save_hero_frame_set` still uses `serde_json::to_string_pretty`, so a meaningful fraction of that file is indentation whitespace that a compact writer would remove for free.
- `06:30 CEST (2026-07-25)` acted on the five findings that review left open. (1) Fixed a real user-visible contradiction in the greenhouse inspect popup: `left_tray`'s `InspectionRef` still read "Small tray site for one future nursery occupant" even though Phase 7 had put a seedling in that exact site (`occupant: Some(FIRST_GREENHOUSE_SEEDLING_ORGANISM_ID)`) — Phase 9 surfaced text written before Phase 7 existed. Reworded to describe the site as holding the nursery's first seedling, and added `occupied_planting_sites_are_not_described_as_awaiting_an_occupant`, which walks every occupied planting site and fails if its inspection text still calls the occupant "future"/"awaiting," so the two cannot drift apart silently again. (2) Switched `save_hero_frame_set` from `serde_json::to_string_pretty` to `to_string`: the regenerated cache dropped **81MB -> 27MB** (-67%) with no behavior change, verified by clearing the cache, rebuilding it through a real run, and re-confirming the hero still renders with dark reds intact (983 color codes, 75 dark-red) from the compact file. (3) Restored the executable bit on `bin/yam` and `bin/yam-sandbox` (`100644` -> `100755` via `git update-index --chmod=+x`); they carry `#!/bin/zsh` shebangs and the other two wrappers were already `100755`, so `./bin/yam` failed with "permission denied" from a fresh clone — masked until now only because `scripts/update.sh`'s `install_wrappers` chmods the *installed* copies. (4) Left the ~8.2MB duplicate hero asset in place deliberately rather than repointing the frozen legacy Python tree for ~4MB, and documented it in `docs/audit.md` with the one rule that matters: if the art changes again, both copies must move together or the Rust and Python pipelines silently diverge on source art. (5) The missing-CI-coverage finding is addressed by opening a PR, since `.github/workflows/verify.yml` only triggers on push to `main` and PRs targeting `main` — a pushed topic branch runs nothing on its own. `bash scripts/verify.sh` clean (275/275, up from 274).
