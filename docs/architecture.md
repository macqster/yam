...
- the hero frame pipeline currently uses Chafa with `--color-space=rgb`, `--color-extractor=median`, `--dither=diffusion`, `--dither-grain=1x1`, and `--fg-only`; `HeroSource` owns each asset's `--bg` cull reference. The default `IVY_VECTOR` contract uses `#336699`. `hero_layer` preserves styled Chafa spans when it copies a frame into the scene grid so the hero does not collapse to monochrome text; see [`rendering.md`](rendering.md) for the full pipeline account and [`chafa-drop-rule.md`](chafa-drop-rule.md) for the source-owned overlap rationale. `LachlanArthur/Braille-ASCII-Art` remains a future renderer reference for explicit braille dot packing and error-diffusion comparison, not an active runtime dependency
