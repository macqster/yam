...
- the live runtime loop now keeps one long-lived `Scene` instance instead of rebuilding the boxed layer list every frame, the layer contract allows obviously hidden overlays to be skipped before `render_to_grid()` is called, the runtime reuses the final composed `Grid` across frames, and the scene renderer now has a reusable per-layer scratch-grid seam adopted by simple active layers plus the lightweight companion projection layers; future render-loop optimization should build from those seams rather than reintroducing per-frame scene construction, empty-grid modal work, or final-frame allocation churn
