...
- `core/world.rs` - owns world bootstrapping by explicit world kind and profile; profile-owned grid, camera defaults, guide plans, population plans, and capabilities keep main-scene, sandbox, greenhouse, and any future lab worlds selected and described intentionally rather than hidden behind one implicit `WorldState::new()` payload
...
- `runtime.rs` - event loop, input, tick, and render orchestration only; its entry contract is `RuntimeOptions` (initial world, hard reset, boot-start policy) rather than a growing positional signature
- boot-start policy is a launch input, not a preference: `ui::state::BootStartPolicy` is carried on `LoadingState`, chosen once in `main` from `--auto-start`/`YAM_AUTO_START`, and never re-read from the environment inside the state machine. Automatic mode reuses `acknowledge_loading_start` rather than duplicating the transition, so `AwaitStart` keeps exactly one exit, and it is recorded in the `boot_start` diagnostics event rather than as a synthetic key event
...
- `WorldKind::Greenhouse` landed (2026-07-21) through the explicit `WorldKind::SELECTABLE` and `WorldKind::profile()` contracts rather than a UI-local toggle, the pattern any future lab world should follow too: `Boot` stays non-selectable, transition labels, coarse `WorldComposition`, grid, camera defaults, guide plans, population plans, and capabilities live on `WorldKind`, and persisted UI snapshots convert through that core contract.
...
- the boot loading sequence is intentionally staged and stateful: blank frame into `coalesce`, then timed loading-bar motion, then a silent wait-for-space hold, then `dissolve` into the first world; this richer sequence applies to startup boot while lightweight world-switch loading may stay shorter and non-blocking
- fresh runtime boot should default to a clean UI session: ordinary launches should not revive persisted dev-mode/modal state on top of the boot screen or the first world before the user explicitly asks for it, regardless of whether the runtime came from the canonical launcher or the direct binary path
- the boot/loading architecture is considered structurally sound enough to keep even though the current ASCII wordmark is still visually unresolved; detailed loading-screen implementation notes and the current pause rationale live in [`loading-screen.md`](loading-screen.md)
...
- the main scene remains the live visualiser/screensaver composition; the greenhouse is now a real separate `WorldKind::Greenhouse` world with a minimal room render, attached seedling state, deterministic growth dispatch, and read-only inspection, while multi-room lifecycle tuning and organism curation remain ahead
...
- the hero frame pipeline currently uses Chafa with `--color-space=rgb`, `--color-extractor=average`, and `--dither=none`, and `hero_layer` preserves the styled spans when it copies the frame into the scene grid so the hero does not collapse to monochrome text; see [`rendering.md`](rendering.md) for the full pipeline account, including the 2026-07-22 fix for the dark-region coverage loss the `average` extractor had. The ditherit-style braille/source-color trial is documented only as a historical experiment because it improved red retention but introduced unacceptable blocking and edge smearing in the face area. `LachlanArthur/Braille-ASCII-Art` is retained as a future renderer reference for explicit braille dot packing and error-diffusion comparison, not as an active runtime dependency
- the hero runtime has two prepared-artifact layers: a validated `HeroPackage` for intentional, provenance-bound compiled output and a disposable `HeroFrameSet` cache for runtime acceleration; both share the structured per-cell `CellGrid` representation
- startup checks a package first, then the frame cache, then falls back to live Chafa. Package validation binds output to schema, preset, geometry, and source-content digest; cache validation remains revision- and mtime-based
- the package/cache migration is complete as a startup concern. Chafa remains the fallback/compiler authority, and future custom-backend or `CellGrid` correction work must preserve the same owner boundaries and terminal-review acceptance gates
- future hero renderer work should stay narrow and test-led; do not combine source, preset, custom-backend, or correction experiments with unrelated compositor, scene, or flora refactors
- REXPaint can be tested through CrossOver as a manual editing node, but `.xp` remains an interchange format behind `CellGrid`; braille usage depends on a controlled font/tile mapping and import/export checks rather than native Unicode semantics
- Ansizalizer/ansipx can be used as a generator reference for custom glyph-set and palette experiments, but its custom-character path is a density ramp rather than true `2x4` braille dot packing, so it should not be treated as a replacement for Chafa-like hero rendering
- the recent hero-rendering test run that produced the `hero-ansipx` preview artifacts was unsuccessful and should remain logged as a cautionary experiment rather than a new baseline
...
- the footer stays compact and split by role instead of trying to be a full legend: on the main scene the left side stays on the stable quit/dev anchor while the right side stays visually quiet with only the version/build stamp; alternate worlds may still pair the global help affordance with that right-side status cluster when the extra discoverability is useful
...
- quit behavior follows that ownership split: `q` now requests a short live-scene shutdown dissolve rather than snapping straight out of the alternate screen; when no persisted tweak state is dirty that dissolve starts immediately, while dirty exits still route through the quit-confirm modal so the user can `save and quit`, `discard and quit`, or `Esc` cancel before the same dissolve handoff runs

## UI / Metamechanics Working Set

- current state: `dev_mode` is the umbrella toggle, `?` opens help, `m` opens move, and `s` opens settings
- current help split: `?` is globally available from the main scene, but the popup should distinguish always-available actions from dev-only tools instead of flattening that boundary
- current move grammar: `Tab` / `Shift+Tab` cycle the active visible target set, arrow keys move that target, and the lower move strip stays modal; the still-reserved `calendar` seam remains a settings-only position row rather than a first-class move-strip target
...
- those seeds are the *starting* composition, not a permanent override: positions saved through dev move mode (`~/.config/yam/state.json`) are preserved on an ordinary launch. They are reseeded only on an explicit `--hard-reset`, or when the saved file's `layout_schema` differs from the running binary's `ui::state::LAYOUT_SCHEMA`, so an upgrade that moved the composition lands on the new arrangement rather than on offsets tuned against the old one. `LAYOUT_SCHEMA` is bumped by hand, and only when `UiOffsets::default()` or the seeded composition actually moves - deliberately not the crate version, which reseeded on every patch release and could never be refreshed on a machine that cannot reach the quit prompt
- saved state reaches disk two ways: the quit-confirm save, and a debounced autosave (`ui::state::autosave_if_due`) that writes once the state has gone `AUTOSAVE_DEBOUNCE` (2s) without a change, so a machine stopped by a signal rather than a keypress can still keep a layout. The autosave is inert while the quit-confirm modal is open, since that modal offers to discard the pending change
- camera state is split into an authored and a rendered pair: `offsets.camera_x`/`camera_y` are what the user set and what is persisted, while `camera.x`/`camera.y` are what is drawn after being fitted to the current terminal. Per-frame work (`clamp_camera`, `sync_camera_to_viewport_center`) writes only the rendered pair - passive rendering must never author state, or merely opening a saved layout in a terminal too small to show it would rewrite it. Only explicit actions author the camera pair, and there are five: the settings editor's camera fields and `recall camera home` set a position outright (`store camera home` writes the separate `camera_home_x`/`_y`, taking it from the *rendered* pair so the bookmark is the view on screen); `step_camera` moves from the rendered position (so a clamped view still answers the first keypress) and re-authors only the axis it steps; leaving follow-hero adopts whatever following put on screen; and a discrete resize re-authors through `preserve_camera_center_on_resize`, which transforms the authored position rather than the rendered one and marks the state dirty instead of changing it silently. Leaving follow-hero has two doors - the `f` key and any arrow key - so both route through one `exit_follow_hero`, which adopts *both* axes and skips adoption entirely when follow never reached a frame
- both the reseed trigger and the authored/rendered split changed on 2026-09-06; `docs/LOG.md` carries why, and through 0.4.4 every launch reseeded, which meant move mode could not persist anything and a later save silently overwrote an earlier hero position with the default
...
- the centered `124x32` follow-hero crop starts at camera `(-62, -16)` - that is a `124x33` *terminal*, since the bottom row is the footer and the camera is fitted to the world rect rather than the frame
...
- hero source assets are owned by `render::hero_source::HeroSource` descriptors; the render path reads canvas size, frame count, requested footprint, cache prefix, minimum frame-0 coverage, and the chafa `absent_color` from there rather than from module constants
- `absent_color` is per-asset because how much of the palette it discards is a per-asset decision: under `--fg-only` chafa never paints it, but it treats resembling art as already drawn and discards it. `IVY` keeps it clear of its palette so nothing is lost; `IVY_VECTOR` overlaps its own palette on purpose, because its flat fills would otherwise render as a solid mass. Distance is a trade rather than a maximum either way, because on partially transparent cells the same value bleeds into chafa's foreground pick
- current registered sources: `IVY_VECTOR`, square `1080x1080` pixels, 48 frames (the default since 0.4.1); and `IVY`, square `820x820` pixels, 64 frames, the previous default, still registered and gated
- source selection is env-only for now: `Hero::new` resolves `YAM_HERO_SOURCE` against the registry and falls back to `DEFAULT`, so a swap is reversible without a rebuild; there is no world-, settings-, or persistence-owned selector yet, and adding one is the open slice
