...
- the live runtime loop now keeps one long-lived `Scene` instance instead of rebuilding the boxed layer list every frame, and the layer contract now allows obviously hidden overlays to be skipped before `render_to_grid()` is called; future render-loop optimization should build from those seams rather than reintroducing per-frame scene construction or empty-grid modal work
