...
- the main scene remains the live visualiser/screensaver composition; the greenhouse is now a real separate `WorldKind::Greenhouse` world (minimal read-only room render, no growth dispatch or flora attachment yet), with multi-room lifecycle tuning and organism curation still ahead
...
- the desired hero-rendering direction is an offline hero-frame compiler that preserves the active Chafa baseline while allowing custom backends to separate monochrome braille shape decisions from color assignment, route manual and scripted corrections through a structured per-cell `CellGrid`, then emit a stable curated `HeroFrameSet` cache for runtime scene rendering
- the first runtime-efficiency implementation step should stop short of a broad hero-pipeline rewrite: keep Chafa as the current visual authority, but move it toward an offline/cache-builder role so ordinary runtime startup can load a prepared `HeroFrameSet` instead of paying GIF decode, temp-file, and process-spawn costs inside `Hero::new()`
- the runtime-facing cache seam for that work is now explicit in `render/hero_cache.rs`, with the matching planning note in [`docs/hero-cache.md`](hero-cache.md); future startup optimization should build on that seam rather than introducing an ad hoc second frame representation
- the first wired runtime use of that seam is now in the hero startup path: `Hero::new()` prefers a prepared `HeroFrameSet` cache in the user cache directory and only drops to the live Chafa compiler path when the cache is missing, stale, or invalid, so ordinary startup no longer has to assume per-frame shelling-out work or write runtime artifacts back into the repo tree
- that cache migration should stay narrow and test-led:
  - define one canonical runtime frame cache format
  - prefer loading cached hero frames on the common path
  - retain the current live Chafa path as an explicit rebuild/fallback seam until the cache is trusted
  - do not combine this first optimization slice with unrelated compositor, scene, or flora refactors
