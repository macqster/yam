...
- `absent_color` is per-asset because it must be a colour the asset does not contain: under `--fg-only` chafa never paints it, but it treats resembling art as already drawn and discards it, so a scene-coloured value silently deletes every dark region
