...
- those seeds are the *starting* composition, not a permanent override: positions saved through dev move mode (`~/.config/yam/state.json`) are preserved on an ordinary launch. They are reseeded only on an explicit `--hard-reset`, or automatically when the saved file's `layout_schema` differs from the running binary's `ui::state::LAYOUT_SCHEMA`, so an upgrade that moved the composition lands on the new arrangement rather than on offsets tuned against the old one. That trigger was the crate version until 2026-09-06, which was too coarse in one direction and unrecoverable in the other: every patch release discarded a tuned layout for a composition that had not changed, and because the stamp is only rewritten by the quit-confirm save, a machine that cannot reach that save — an appliance whose launcher stops YAM with a signal — could never refresh it, so every boot reseeded and no position could survive at all. `LAYOUT_SCHEMA` is bumped by hand, and only when `UiOffsets::default()` or the seeded composition actually moves. Saved state is written by a debounced autosave as well as by the quit-confirm save: `ui::state::autosave_if_due` writes once the state has gone `AUTOSAVE_DEBOUNCE` (2s) without a change, so a machine that is stopped by a signal rather than a keypress can still keep a layout. It is inert while the quit-confirm modal is open, since that modal offers to discard the pending change. Note that `clamp_camera` rewrites `offsets.camera_x`/`camera_y` every frame to fit the current terminal without marking the state dirty, so whichever save runs next persists the *clamped* camera rather than the raw tuned value - long-standing behavior, but reachable without quitting now that autosave exists. Through 0.4.4 every launch reseeded, which meant move mode could not persist anything and a later save silently overwrote an earlier hero position with the default
