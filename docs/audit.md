...
- [medium] The first render-loop reuse, hidden-layer skip, final-buffer reuse, and initial scratch-grid slices are now in place: runtime keeps one long-lived `Scene`, no longer asks obviously closed modal/help/quit layers to allocate empty grids, reuses the final composed `Grid` across frames, and can now reuse scratch grids for simple active layers, the lightweight companion projection layers, and the hero layer. The remaining hot-path seam is widening that per-layer scratch reuse to the remaining heavier always-active layers, especially `debug` and possibly `vines`.
