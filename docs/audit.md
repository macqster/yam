...
2. Hero-rendering pipeline: the dark-color defect is fixed and package-first
   startup is live. The remaining risk is keeping source, preset, package,
   cache, and visible-terminal review aligned whenever the art or renderer
   changes; structural validation alone still cannot approve visual fidelity.
   See `hero-revision.md` and `hero-package.md`.
3. Flora runtime: the first vine prototype is live through deterministic growth and leaf hosting, `core::organism` provides the first shared identity/species-registry/journal vocabulary, multi-family storage is now locked as an enum-backed `FloraInstance` family store (2026-07-21), and growth/aging dispatch now iterates every vine instance rather than one hard-coded id (2026-07-21). A second family, `Seedling`, landed 2026-07-22 and shares the identity vocabulary plus the cadence-gate and all-instances dispatch shape — but not a growth *rule*, and deliberately so: seedling growth is a life-state stage advance while vine growth is geometric mutation of axes and segments, and `SeedlingInstance` carries no geometry at all. So the rules stay per-family because of the second family's shape, not because it is missing; a family that does carry geometry is what would put a shared rule in question.
...
- `low` `scene_config.json` is active for tooling and should stay aligned with the tooling defaults if they change. Its cross-language coupling was cut on 2026-09-02: no Rust code reads, embeds, or asserts on it, and it no longer triggers Rust rebuilds through the launcher wrappers. The one live reader is `tools/experiments/config.py`, reachable only by running `python3 app.py` by hand; its loader now resolves the tracked `gif_path` through the repository's `tools/legacy-python/` compatibility root regardless of the caller's working directory. The legacy fallback renderer uses the current Rust Chafa preset semantics (`rgb`/`median`/diffusion/`#00e000`) when the historical Go helper is unavailable, but it remains a reference/tooling path rather than a second Rust runtime owner.
...
- `low` `core::spatial` is the sole spatial resolver; `scene::coords` (its `ScreenPos` alias, `project_world_to_screen`, `resolve_element_screen_position`, and the `Space`/`EntityId`/`Element` compatibility dispatch) was retired 2026-07-21 after confirming zero call sites outside its own tests, and the matching `scripts/check.sh` isolation guard was removed with it. `scene/entity.rs` still names its attachment structs domain-specifically (`EntityPose`/`AttachedEntityPose`) rather than constructing `SpatialAnchor`/`SpatialAttachment` directly, but its math already delegates to `SpatialResolver` — a naming duplication, not a routing split. Entity-backed anchor lookup is exclusively `core::spatial::SpatialAnchorLookup`, implemented on `WorldState`. See `docs/LOG.md` for the retirement's full account.
  - evidence: `src/core/spatial.rs`, `src/scene/entity.rs`, `src/core/guide.rs`, `src/render/guide.rs`, `src/scene/render_state.rs`, `scripts/check.sh`
- `low` Persisted UI state has one writer path per concern again. Two per-frame functions - `clamp_camera` and `sync_camera_to_viewport_center` - wrote `offsets.camera_x`/`camera_y`, the pair that gets saved, so rendering authored state: opening a saved layout in a terminal that could not show it rewrote it on the first frame. Harder to notice while the quit-confirm save was the only way to disk; the debounced autosave (2026-09-06) made it a silent permanent loss on the next unrelated edit. Both now write the rendered `camera.x`/`camera.y` only, and authoring is limited to explicit actions. The reseed trigger moved off `CARGO_PKG_VERSION` to `ui::state::LAYOUT_SCHEMA` in the same pass, since keying it on the crate version discarded tuned layouts on every patch release and could never be refreshed on a machine that cannot reach the quit prompt.
  - evidence: `src/ui/state.rs`, `src/runtime.rs`, `docs/architecture.md`
- `low` Flora storage is closed: `FloraState` stores an enum-backed `FloraInstance` family store (`organisms: Vec<FloraInstance>`, carrying `Vine(VineInstance)` and, since 2026-07-22, `Seedling(SeedlingInstance)`) instead of a bespoke `vines` field, with every call site migrated. Growth dispatch (`systems::growth::run_growth`) now iterates every vine instance rather than one hard-coded id, matching `run_aging`, and `run_greenhouse_growth` follows the same shape for seedlings; the growth *rules* stay per-family by design, sharing the cadence gate and all-instances iteration rather than a rule body. See `docs/LOG.md` for the fix history.
...
- `low` The hero-rendering pipeline now uses package-first startup: a validated `HeroPackage` is preferred, followed by the disposable `HeroFrameSet` cache and then live Chafa compilation. The package/compiler path is structurally validated and has passed bounded live use, but any future source, preset, timing, or cell-content change still requires real-terminal visual acceptance.
  - evidence: `src/render/chafa.rs`, `src/render/hero_package.rs`, `docs/hero-package.md`, `docs/rendering.md`
...
- `low` The Phase 1 offline hero compiler landed 2026-08-20 from `agent/hero-revision-contract`, where it had sat unmerged since 2026-07-27. `render::hero_package` is the validated versioned artifact, `render::hero_manifest` records provenance (SHA-256 source digest, geometry, per-frame timing, compiler id/version, preset id, literal chafa args), and `render::hero_compiler` is reachable as `yam-rust --compile-hero` and never on the ordinary startup path. It was adapted on landing rather than merged as authored: the branch predated per-source descriptors, so `CompileOptions` now takes geometry and `absent_color` from a `HeroSource` and honors `YAM_HERO_SOURCE`, and `chafa_preset_args` carries the source's own drop reference. Runtime package wiring landed later the same day: startup now prefers a matching validated package, then the disposable frame cache, then live Chafa compilation.
- evidence: `src/render/hero_compiler.rs`, `src/render/hero_package.rs`, `src/render/hero_manifest.rs`, `docs/hero-package.md`
- `low` The `absent_color` contract is self-policing: `absent_color_is_actually_absent_from_every_source` requires separation unless a source has an explicit overlap entry with its worst-frame count pinned. `IVY_VECTOR` is the deliberate default exception at 259464 pixels within the 128-unit Euclidean RGB radius for `#336699`; `IVY` remains separated. The pin protects the intended cull shape while the normal gate prevents accidental overlap for other sources.
  - evidence: `src/render/chafa.rs`
- `low` **Accepted 2026-09-10, canonicalized after iMac divergence.** The
  default `hero_gif_2` contract uses `--color-extractor=median`,
  `--dither=diffusion`, `--dither-grain=1x1`, and the deliberate `#336699`
  source-owned cull reference. It supersedes the 2026-09-09 non-overlapping
  `#00e000` policy. iMac x86_64 and MBP arm64 Chafa 1.18.2 binaries produced
  different frame payloads despite identical GIF digest, reported version, and
  literal arguments; iMac's local package visibly formed a dark mass. The
  accepted MBP payload is therefore stored as a checksum-verified, compressed
  canonical default package and takes precedence over machine-local output.
...
- `low` **Unified 2026-09-15.** Every registered hero source now declares a
  checksum-verified canonical package. The updater's registry-wide compile
  therefore cannot create competing host-local output for either source;
  local packages and revision-keyed frame caches remain fallbacks for source
  development or a missing/invalid tracked artifact. The default package
  remains the MBP-reviewed fleet authority, while the legacy package closes
  the previously different behavior of `YAM_HERO_SOURCE=hero_gif_1` across
  architectures.
...
- `low` **Resolved 2026-09-09.** The legacy Python compatibility path `tools/legacy-python/hero/assets/hero_go.gif` is now a tracked symlink to the single Rust-owned payload `assets/hero_gif_1.gif`. Existing legacy defaults keep their historical path, while the repository no longer carries two independently editable 4.1MB GIF copies that could silently diverge. The symlink uses a repository-relative target so it remains valid after the checkout moves between machines.
  - evidence: `assets/hero_gif_1.gif`, `tools/legacy-python/hero/assets/hero_go.gif`, `tools/legacy-python/runtime/system.py`
- `low` The hero frame cache is written with compact JSON (`serde_json::to_string`, not `to_string_pretty`), which cut the generated file from 81MB to 27MB with no behavior change; the file is machine-only, so indentation was pure overhead. Its key includes a renderer revision as well as the dimensions, so renderer-only changes cannot reuse stale frames; GIF mtime still governs freshness within a revision. Warm cache load measured at ~150ms against ~4300ms for live `chafa` compilation (2026-07-25). The cache is now the second prepared-artifact choice after a validated package, and still meets its fallback acceptance bar.
...
1. Hero visual-acceptance and revision contract
