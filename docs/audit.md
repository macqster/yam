...
- `low` The central companion projection path now returns signed `ScreenPos` values through `project_world_to_screen(...)` and `RenderState::{clock,weather,date,calendar}_screen()`, and hero, guide, and vine rendering now use the same typed projection helper instead of the old world-shaped screen wrapper. The migration is still incomplete because the debug projection bundle and `resolve_position(...)` bridge keep compatibility paths for older call sites.
  - evidence: `src/scene/coords.rs`, `src/render/render_state.rs`, `src/scene/layers/hero_layer.rs`, `src/render/guide.rs`, `src/scene/layers/vine_layer.rs`
