...
- `low` `scene_config.json` is active for tooling and should stay aligned with the tooling defaults if they change. Its cross-language coupling was cut on 2026-09-02: no Rust code reads, embeds, or asserts on it, and it no longer triggers Rust rebuilds through the launcher wrappers. The one live reader is `tools/experiments/config.py`, reachable only by running `python3 app.py` by hand; note that its `gif_path` (`hero/assets/hero_go.gif`) resolves only when the process CWD is `tools/legacy-python/`, and that the legacy tree's renderer shells out to a `go run ./cmd/yamv2` target that does not exist in this repo, so that path fails silently either way. Not repaired here - the frozen legacy tree is out of scope for a stabilization batch.
