...
- `low` The central companion projection path now returns signed `ScreenPos` values through `project_world_to_screen(...)` and `RenderState::{clock,weather,date,calendar}_screen()`, reducing the old world-shaped screen leakage. The migration is still incomplete because raw guide, vine, and debug line drawing continue to use the legacy `world_to_screen(...) -> WorldPos` compatibility wrapper, and `resolve_position(...)` remains a bridge for older call sites.
  - evidence: `src/scene/coords.rs`, `src/render/render_state.rs`, `src/scene/layers/clock_layer.rs`, `src/scene/layers/weather_layer.rs`, `src/scene/layers/date_layer.rs`
