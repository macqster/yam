...
- `low` **Fixed 2026-07-21**: found the same shape of risk elsewhere via a broader sweep (find-by-id resolvers, direct array indexing, `.unwrap()`/`panic!()` in production code) prompted by the `GreenhouseState::active_room()` fix. `systems::fields::update_fields()` indexed `world.fields.density[idx]` using `world.grid.index(entity.x, entity.y)` with zero bounds-check against the entity's position — a real panic risk on the main per-frame tick path once anything ever pushes an `Entity` with an out-of-grid position (`world.entities` is currently always empty in production, so this was latent, not live, exactly like the greenhouse case before its fix). Fixed by skipping entities outside `[0, width) x [0, height)` before indexing, with two new regression tests (one confirming an out-of-bounds `u16::MAX` position no longer panics — reproduced the exact pre-fix panic first to confirm the test was meaningful, then verified the fix silences it — and one confirming an in-bounds position still marks density correctly). The rest of the sweep came back clean: `.unwrap()`/`panic!()` outside tests is limited to two lines already inside a test module; every other find-by-id resolver (`core/world.rs`, `core/guide.rs`) already returns `Option`; and every other direct grid/array index in production code (`render/compositor.rs`, `render/mask.rs`, `render/drawing.rs`, `scene/layers/field_layer.rs`) was already bounds-checked immediately before use.
