...
- `low` The spatial model is now unified on `core::spatial` as the sole resolver: `scene/coords.rs` was deleted 2026-07-21 after confirming zero call sites outside its own tests (verified twice, by two independent greps for `crate::scene::coords` and the bare `coords` token repo-wide). `scene/entity.rs` still expresses attachment composition with its own domain-named structs (`EntityPose`/`AttachedEntityPose`) rather than constructing `core::spatial::SpatialAnchor`/`SpatialAttachment` directly, but its math already delegates to `SpatialResolver`, so this is a naming/shape duplication, not a routing split; `core/guide.rs` (storage) and `render/guide.rs` (render-only consumer via `SpatialGuideIndex`) are intentionally separate by design per `docs/architecture.md`, not an unresolved split.
  - evidence: `src/core/spatial.rs`, `src/scene/entity.rs`, `src/core/guide.rs`, `src/render/guide.rs`
- `low` The core/systems ownership boundary guard in `scripts/check.sh` is unchanged and still fails if `src/core` imports `crate::scene::` or if `src/systems` imports scene, render, UI, or terminal modules; the separate `crate::scene::coords` isolation guard was removed 2026-07-21 along with the module it guarded, since a retired module has nothing left to isolate.
...
- `low` `RenderState` companion helpers, active companion screen consumers, hero rendering, debug rendering, guide rendering, and vine drawing project through or consume `core::spatial` directly and use signed `core::spatial::SpatialScreenPoint` values, with no remaining compatibility projection path: `scene::coords` (including its `ScreenPos` alias, `project_world_to_screen(...)`, and `resolve_element_screen_position(...)`) was deleted 2026-07-21. Hero row placement, debug marker writes, and shared drawing writes use checked signed-to-grid conversion. Entity-backed anchor lookup routes through `core::spatial::SpatialAnchorLookup`, implemented directly on `WorldState`.
  - evidence: `src/core/spatial.rs`, `src/render/render_state.rs`, `src/scene/layers/debug_layer.rs`, `src/scene/layers/hero_layer.rs`, `src/render/guide.rs`, `src/scene/layers/vine_layer.rs`
...
- `low` `Space::Anchor(EntityId)` and the rest of the `scene::coords` compatibility dispatch (`Space`, `EntityId`, `Element`) no longer exist; entity-backed anchor lookup is now exclusively `core::spatial::SpatialAnchorLookup`, implemented on `WorldState`, with every consumer (`core::world`, `scene::entity`, `ui::state`) calling it directly.
  - evidence: `src/core/spatial.rs`, `src/core/world.rs`, `src/scene/entity.rs`, `src/ui/state.rs`
...
- `low` **Superseded 2026-07-21**: the previous entry here claimed 12 transitive crates carried duplicated major/minor versions, inferred by manually reading `Cargo.lock` text in a sandbox without `cargo` available. A real `cargo tree -d` run (this pass, real toolchain) reports "nothing to print" — a genuinely clean dependency graph, matching the original 2026-05-31 reading. The manual-grep inference was inaccurate; trust `cargo tree -d` output over text-based `Cargo.lock` inspection going forward when a toolchain isn't available to verify a claim like this directly. Note: `cargo tree -d` only inspects the normal dependency graph by default; `syn` genuinely exists at both `1.0.109` and `2.0.117` in `Cargo.lock`, but that pair is confined to `build.rs`'s `built` crate (a build-dependency), invisible to the default view and not shipped in the binary — this is why the "clean" reading and the raw `Cargo.lock` text can look contradictory at a glance.
- `low` Ran the dependency update this file previously recommended: `cargo update -p ratatui -p chrono -p serde -p serde_json -p unicode-segmentation -p tachyonfx`. Five of six bumped as expected (`ratatui` 0.30.0→0.30.2, `chrono` 0.4.44→0.4.45, `serde_json` 1.0.150→1.0.151, `unicode-segmentation` 1.13.2→1.13.3, `tachyonfx` 0.25.0→0.25.1); `serde` stayed at `1.0.228` even though `1.0.229` is available (confirmed via `cargo update -p serde --dry-run`) because bumping it alone pulls in a new `syn 3.0.2` on top of the already-duplicated `syn` 1.x/2.x pair — left alone deliberately rather than deepening existing drift for a patch-only bump with no functional need. The `ratatui` bump itself introduced one genuinely new normal-graph duplicate not present before this pass: `hashbrown` at `0.16.1` (via `kasuari`, ratatui's layout engine) and `0.17.1` (via the newly-bumped `lru` 0.18.1, a `ratatui-core` dependency) — confirmed via `cargo tree -d` before/after and `git diff Cargo.lock`. This is upstream drift inside `ratatui`'s own tree, not something `yam-rust`'s `Cargo.toml` controls; not worth chasing. `cargo build --release`, `bash scripts/verify.sh` (255 tests), and `./target/release/yam-rust --identity` all confirmed clean after the update.
  - evidence: `Cargo.lock`, `Cargo.toml`
- `low` `GreenhouseState::active_room()` contradicted the project's established no-panic pattern (see the `chafa.rs` placeholder-frame precedent above): its invariant is only enforced in `GreenhouseState::new`, but `rooms` and `active_room_id` are public fields that can be mutated independently afterward, so a stale/mismatched `active_room_id` could reach the `.expect()` at runtime. Fixed in this pass: it now returns `Option<&GreenhouseRoom>` instead of panicking. Not a live risk before this fix since `core::greenhouse` is not yet wired into `WorldState`, but worth closing before greenhouse work goes live.
  - evidence: `src/core/greenhouse.rs`
- `low` **Fixed 2026-07-21**: found the same shape of risk elsewhere via a broader sweep (find-by-id resolvers, direct array indexing, `.unwrap()`/`panic!()` in production code) prompted by the `GreenhouseState::active_room()` fix. `systems::fields::update_fields()` indexed `world.fields.density[idx]` using `world.grid.index(entity.x, entity.y)` with zero bounds-check against the entity's position — a real panic risk on the main per-frame tick path once anything ever pushes an `Entity` with an out-of-grid position (`world.entities` is currently always empty in production, so this was latent, not live, exactly like the greenhouse case before its fix). The first repair skipped entities outside `[0, width) x [0, height)`, but review found a second reachable invariant gap because the three public field buffers can be mutated to lengths that do not match the public grid dimensions. `update_fields()` now repairs every field buffer to the exact grid area before clearing or indexing it, and three regressions cover out-of-bounds entities, normal in-bounds density, and mismatched field storage. The rest of the sweep came back clean: `.unwrap()`/`panic!()` outside tests is limited to two lines already inside a test module; every other find-by-id resolver (`core/world.rs`, `core/guide.rs`) already returns `Option`; and every other direct grid/array index in production code (`render/compositor.rs`, `render/mask.rs`, `render/drawing.rs`, `scene/layers/field_layer.rs`) was already bounds-checked immediately before use.
  - evidence: `src/systems/fields.rs`
- `low` **Fixed 2026-07-21**: the deterministic weather-test seam removed live-network flakiness but initially bypassed the production background worker and channel entirely. Weather fetch behavior is now injected as a function pointer while both test and production refreshes use the same spawned worker and receiver path; focused success and failure tests cover cached snapshot population and stale fallback without contacting `wttr.in`.
  - evidence: `src/ui/state.rs`, `docs/weather-widget.md`
- `low` **Re-verified 2026-07-21** (closing the "future pass with `cargo`" note below): a brace-aware re-scan still finds the same 19 non-test `.expect()`/`unreachable!()` call sites (`src/runtime.rs:50`, `src/render/hero.rs:92`, `src/render/fonts.rs:80`, and 16 in `src/weather/atlas.rs`), and this pass traced each invariant's actual source with a real toolchain rather than inferring from text alone: `runtime.rs:50`'s `unreachable!()` is guarded by `UiState::effect_phase()`'s exhaustive match, which structurally can only return `Some` for the two phases `build_loading_effect` handles: `hero.rs:92`'s frame-list non-emptiness is guaranteed by both `Hero` constructors (`new()`/`test_stub()`), and a repo-wide grep confirmed `Hero` is never constructed via struct-literal outside `hero.rs` and `hero.frames` is never mutated externally (read-only elsewhere), so the invariant can't currently be violated post-construction; `fonts.rs:80`'s fallback FIGlet only panics if three independent parses of a compile-time `include_str!`-embedded bundled font asset all fail; and the `weather/atlas.rs` sites index fixed-width compile-time sprite string constants whose row counts are covered by existing atlas tests. No `debug_assert!` narrowing was applied — each site is a hard invariant enforced by callers or compile-time data, not a soft assumption worth downgrading. Left unchanged; this note can be considered closed rather than open unless a future change (e.g. exposing `Hero::frames` to external mutation) reopens the question.
  - evidence: `src/runtime.rs`, `src/render/hero.rs`, `src/render/fonts.rs`, `src/weather/atlas.rs`
- `low` A tooling/dependency freshness check on 2026-07-21 (docs.rs version pages, since crates.io and rustup are blocked from this review sandbox) found six direct dependencies with newer patch releases already inside the existing `Cargo.toml` semver ranges, so no `Cargo.toml` edit is needed, only a lockfile refresh: `ratatui` 0.30.0 → 0.30.2, `chrono` 0.4.44 → 0.4.45, `serde` 1.0.228 → 1.0.229, `serde_json` 1.0.150 → 1.0.151, `unicode-segmentation` 1.13.2 → 1.13.3, `tachyonfx` 0.25.0 → 0.25.1. `crossterm`, `image`, `tempfile`, `unicode-width`, `figlet-rs`, and `ansi-to-tui` are already at their current latest release. No lockfile edit was made this pass for the same reason as the dependency-graph note above (no `cargo` to verify a hand-edit); run `cargo update -p ratatui -p chrono -p serde -p serde_json -p unicode-segmentation -p tachyonfx && cargo test` locally. `rust-toolchain.toml` (`channel = "stable"`, currently resolving to 1.97.1) and the unversioned `Brewfile` formulas already track latest by design and need no change. A Rust 2024 edition migration is available (2024 stabilized at 1.85, well below the current stable toolchain) but is discretionary polish, not a requirement; left untouched.
  - evidence: `Cargo.toml`, `Cargo.lock`, `rust-toolchain.toml`, `Brewfile`
- `low` **Fixed 2026-07-21**: `cargo clippy --all-targets -- -D warnings` previously failed on a clean `main` checkout (confirmed pre-existing via `git stash` + re-run before fixing, unrelated to the spatial-consolidation pass) with 3 `items-after-test-module` errors (`src/render/chafa.rs:99`, `src/render/hero.rs:298`, `src/scene/layers/hero_layer.rs:193` — each file's `#[cfg(test)] mod tests` sat before trailing production items) and 1 `useless-conversion` error (`src/runtime.rs:459`, a redundant `.into()` on an already-`Duration` value). Fixed by moving each file's test module to the end of the file (pure reorder, verified byte-identical non-blank content via diff before and after) and removing the redundant `.into()`. `scripts/check.sh` and `bash scripts/verify.sh` both now pass end-to-end, satisfying the "verification green" gate `TODO.md` and `docs/greenhouse-roadmap.md` require before broad feature work.
  - evidence: `src/render/chafa.rs`, `src/render/hero.rs`, `src/scene/layers/hero_layer.rs`, `src/runtime.rs`
  - evidence: `src/render/hero.rs`, `src/runtime.rs`, `src/scene/layers/hero_layer.rs`
