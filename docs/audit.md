...
- `low` The hero-rendering pipeline is still experiment-heavy outside the active Chafa path: the offline compiler / `CellGrid` direction is locally compiled, structurally validated, and has reached the main scene in a bounded smoke, but runtime package wiring and full visual acceptance remain open.
...
- `low` The 2026-07-27 hero ground-up rebuild batch is locally formatted,
  compiled, structurally tested, and exercised through a bounded live smoke.
  Runtime package wiring and the full dark-color visual acceptance remain
  open, separate from the pre-existing fidelity risk above.
  - evidence: `src/render/chafa.rs`, `src/render/hero_compiler.rs`, `src/render/hero_package.rs`, `src/render/hero_manifest.rs`, `src/render/cell_grid.rs`, `docs/hero-package.md`
