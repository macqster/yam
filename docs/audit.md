...
full stage-by-stage trace, measured numbers, per-stage evaluation, and ranked
opportunities. Read it before starting rework rather than re-deriving the
pipeline.

That document's Resolution Points section carries six open decisions, `HQ-1`
through `HQ-6`, each with its own resolution criteria and owning doc. They are
deliberately left open to be settled *during* the rework phase, not before it;
none is pre-judged, here or there. `HQ-3` (whether `chafa` stays a subprocess)
sequences the others, and `HQ-5` (whether the source art shape changes) moves the
