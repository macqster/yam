...
- run `scripts/check-docs.sh` for docs-hygiene batches; it enforces the active-doc inventory, `TODO.md` -> `known_issues.md` issue-link consistency, the canonical `Cargo.toml` -> `README.md` current-release version match, and README local-asset path validity directly, and it also runs repo-configured `cspell` / `markdownlint` checks when those CLIs are available in the local environment
