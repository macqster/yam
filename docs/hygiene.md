...
- `markdownlint-cli2` and `cspell` are npm packages, not part of `Brewfile`; install them once per machine with `npm install -g markdownlint-cli2 cspell` (matching CI's `.github/workflows/verify.yml` setup) so `scripts/check-docs.sh` actually runs those checks locally instead of silently skipping them when the CLIs are absent
