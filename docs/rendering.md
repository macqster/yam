...
- The first render-loop allocation cleanup slice is also now live: runtime keeps one long-lived `Scene` and reuses the boxed layer stack across frames instead of rebuilding it on every draw. The next conservative step on that path is buffer reuse, not another scene-construction rewrite.
