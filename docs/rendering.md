...
- GIF subimage frames are expanded onto a full `820x820` logical canvas before chafa rendering so partial frames, including frames 15 and 30, cannot stretch vertically. The canvas is transparent (not an opaque matte fill): the source GIF (`hero_go.gif` provenance, swapped in 2026-07-22) carries real per-pixel alpha, and the pipeline now preserves it end to end into the temp PNG chafa reads, instead of compositing every frame onto a flatten color.
