...
- Two tests gate any asset swap: `every_hero_source_matches_its_declared_geometry` fails when a descriptor's frame count or canvas size disagrees with the actual file, and `hero_frames_keep_a_transparent_canvas_rather_than_a_matte` fails if the alpha contract is lost again.
