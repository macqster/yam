...
- modal geometry is terminal-bounded: a terminal narrower than a panel's border-and-gutter footprint yields a zero-width body and clipped content, never unsigned-width wrapping or a render panic; normal-size panel geometry remains unchanged
...
- The historical 2026-08-19 measurement recorded `hero_gif_1` frame 0 at 1918/4608 cells (41.6%, floor 20%) and `hero_gif_2` at 923/4608 (20.0%, floor 10%) under the deliberate `#336699` vector cull. The 2026-09-09 non-overlapping `#00e000` experiment measured 1725/4608 (37.4%) but is no longer the default policy. Coverage is a structural floor, not a visual preference metric: the accepted default deliberately favors the `#336699` keep/cull shape and requires real-terminal review for any future change.
...
- The hero frame pipeline uses Chafa with `--color-space=rgb`, `--color-extractor=median`, `--dither=diffusion`, `--dither-grain=1x1`, and `--fg-only`; `--bg` is derived from that source's `HeroSource::absent_color`. **`--bg` is not a background fill and is never painted under `--fg-only`** — it is the colour Chafa treats as *already on screen*, so art resembling it is dropped rather than drawn. [`chafa-drop-rule.md`](chafa-drop-rule.md) documents the mechanism, measurements, and source-owned exceptions.
  - `IVY` uses `#00e000`, a colour absent from its palette, so its art is not deliberately culled.
  - `IVY_VECTOR`, the default hero, uses `#336699` deliberately. Its flat fills otherwise render as a solid braille mass under `--fg-only`; the blue cull retains the desired open silhouette. The significant overlap is pinned by the source test, so asset or cull-policy drift fails verification rather than changing the default silently.
- `hero_layer` preserves the styled spans when it copies the frame into the scene grid so the hero does not collapse to monochrome text. Investigation (2026-07-22) found the prior `average` extractor, combined with the then-flattened opaque canvas, was dropping roughly 80% of the frame grid as "no coverage" rather than merely desaturating it: dark, low-contrast regions (not just dark reds) were being judged close enough to the assumed background to skip entirely. Preserving real source alpha plus switching to `median` fixed both problems together — `median` alone (kept against the old flattened canvas) recovered coverage by painting the true background too, which is not correct either. The current `median` choice remains the shared runtime/compiler contract; a later comparison against the restored alpha-carrying canvas found that `average` produced the same cells for the shipped sources in that measurement, but it is historical evidence, not the active preset. The 2026-07-22 note is history, not current configuration. The ditherit-style braille/source-color trial remains documented in the log as a rejected alternative because it improved red retention but introduced unacceptable blocking and edge smearing in the face area.
...
- Hero startup first uses a checksum-verified canonical package when the source
  ...
  canonical package is reviewed source state, the local package is intentional
  machine output, and the cache is runtime acceleration.
