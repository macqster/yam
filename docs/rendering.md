...
- Two sources are registered:
  - `IVY` (`assets/hero_gif_1.gif`, `820x820`, 64 frames, requested at `96x48` cells) — the original cel-derived art, and still `hero_source::DEFAULT`.
  - `IVY_VECTOR` (`assets/hero_gif_2.gif`, `1080x1080`, 48 frames, requested at `96x48` cells) — the same character and pose cycle redrawn as flat vector art in Moho, added 2026-08-19.
- `IVY_VECTOR` is a registry *probe*, not a hero swap: `hero_source::DEFAULT` is unchanged, so an ordinary launch still renders `IVY`. Its job is to make the registry carry more than one real asset, so the swap gates below run against genuinely different art instead of one file checked against itself.
- `YAM_HERO_SOURCE=<stem>` selects any registered source by stem for one launch (`hero_source::resolve_from_env`, read once in `Hero::new`). This is deliberately the smallest form of the selection surface — enough to look at candidate art in the running app before anything commits to it, not a settings contract. An unset or unknown stem resolves back to `DEFAULT`, because a typo in an env var should cost the reader their probe rather than their hero. A world- or settings-owned selector, and any persistence of that choice, remain the next slice.
...
- Measured 2026-08-19 against chafa 1.18.2, through the test's own `covered_cells` helper: `hero_gif_1` frame 0 covers 932/4608 cells (20.2%, floor 20%), `hero_gif_2` covers 706/4608 (15.3%, floor 10%). The 41.5% figure this section previously cited for `hero_gif_1` does not reproduce and has been withdrawn — which means `IVY`'s floor currently has 0.2 points of headroom rather than the roughly 2x the original note assumed. That floor is deliberately left unchanged pending a decision on recalibrating it; see `docs/audit.md`.
...
- The hero frame pipeline uses Chafa with `--color-space=rgb`, `--color-extractor=median`, and `--dither=none`; `--bg` is derived from the `HERO_DISPLAY_BG` constant, chafa's display-background hint (no longer a canvas-fill color). `hero_layer` preserves the styled spans when it copies the frame into the scene grid so the hero does not collapse to monochrome text. Investigation (2026-07-22) found the prior `average` extractor, combined with the then-flattened opaque canvas, was dropping roughly 80% of the frame grid as "no coverage" rather than merely desaturating it: dark, low-contrast regions (not just dark reds) were being judged close enough to the assumed background to skip entirely. Preserving real source alpha plus switching to `median` fixes both problems together — `median` alone (kept against the old flattened canvas) recovered coverage by painting the true background too, which is not correct either. The ditherit-style braille/source-color trial remains documented in the log as a rejected alternative because it improved red retention but introduced unacceptable blocking and edge smearing in the face area.
