...
- The first render-loop allocation cleanup slices are now live: runtime keeps one long-lived `Scene`, reuses the boxed layer stack across frames, skips obviously closed modal/help/quit layers before asking them to allocate grids at all, and now also reuses the final composed `Grid` across frames in the live runtime loop. The next conservative step on that path is per-layer buffer reuse, not another scene-construction rewrite.
