...
- The first render-loop allocation cleanup slices are now live: runtime keeps one long-lived `Scene`, reuses the boxed layer stack across frames, and skips obviously closed modal/help/quit layers before asking them to allocate grids at all. The next conservative step on that path is buffer reuse, not another scene-construction rewrite.
