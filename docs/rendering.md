...
- Four tests gate any asset swap, and all four iterate `hero_source::ALL` rather than only the default: `absent_color_is_actually_absent_from_every_source` fails when a source's palette comes within 128 (Euclidean RGB) of its own `absent_color`, since that is the distance below which chafa starts discarding the art as already-painted; `every_hero_source_matches_its_declared_geometry` fails when a descriptor's frame count or canvas size disagrees with the actual file, `hero_frames_keep_a_transparent_canvas_rather_than_a_matte` fails if the alpha contract is lost again, and `rendered_hero_frames_contain_real_content_not_placeholders` fails when a live render produces placeholder frames or collapses below that source's declared frame-0 coverage floor.
...
- Measured 2026-08-19 against chafa 1.18.2, through the test's own `covered_cells` helper, after the `absent_color` fix: `hero_gif_1` frame 0 covers 1923/4608 cells (41.7%, floor 20%), `hero_gif_2` covers 1725/4608 (37.4%, floor 10%). Before that fix the same measurement gave 932 (20.2%) and 706 (15.3%) — the pipeline was discarding a little over half of each hero.
