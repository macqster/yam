...
- Measured 2026-08-19 against chafa 1.18.2, through the test's own `covered_cells` helper: `hero_gif_2` frame 0 covers 706/4608 cells (15.3%, floor 10%), `hero_gif_1` covers 932/4608 (20.2%, floor 20%). The new default is the sparser of the two by art style — flat fills and clean line work light fewer dots than cel shading — which is expected, not a regression. The 41.5% figure this section previously cited for `hero_gif_1` does not reproduce and has been withdrawn, which leaves that source's floor with 0.2 points of headroom rather than the roughly 2x the original note assumed. That floor is deliberately left unchanged pending a decision on recalibrating it; see `docs/audit.md`.
