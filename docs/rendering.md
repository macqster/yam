...
- The hero frame pipeline uses Chafa with `--color-space=rgb`, `--color-extractor=average`, `--dither=none`, and `--fg-only`; `--bg` is derived from that source's `HeroSource::absent_color`. **`--bg` is not a background fill and is never painted under `--fg-only`** — verified: chafa emits zero background codes at every `--bg` value tested. It is the colour chafa treats as *already on screen*, so any art resembling it is judged redundant and dropped rather than drawn. [`chafa-drop-rule.md`](chafa-drop-rule.md) documents the mechanism, the measurement harness, and the procedure for choosing this value when new art is registered. Both registered sources currently use separated values selected to preserve their authored dark regions while limiting edge bleed:
  ...
  - `IVY_VECTOR` uses `#00e000`, a colour absent from its palette. This is the least-clearance value measured that clears the drop radius for both sources, so the dark red, leggings, and line art remain visible while edge bleed stays bounded. The resulting frame-0 coverage is 1725/4608 cells (37.4%), rather than the 923/4608 cells produced by the old deliberately overlapping `#336699` policy.
