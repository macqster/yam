...
- the first live scaffold runtime slice is now a world-owned static support shape in `core::scaffold`: a rear-only seat cradle, back brace, leg brace, and fork mass drawn through a read-only layer beneath the hero; optional foreground nesting edges remain future composition work rather than parked runtime abstraction
...
- sandbox is now also the preferred scaffold-prototyping room: hero, companions, and scaffold may be made visible there through UI-owned prototype toggles so composition trials can reuse the live pipeline without forcing every experiment into the main scene
