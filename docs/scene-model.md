...
- the greenhouse is now a real separate `WorldKind::Greenhouse` simulation world (minimal read-only room render); multiple rooms, labs, and multiple flora species with their own anatomy, morphology, and growth behavior remain future work on top of that world
