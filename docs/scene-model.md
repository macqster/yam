...
- the compatibility layer in `scene/coords.rs` now exposes signed `ScreenPos` and `project_world_to_screen(...)` for central companion projection, while keeping `world_to_screen(...)` as a legacy world-shaped wrapper for raw guide/vine/debug line paths that still need migration
