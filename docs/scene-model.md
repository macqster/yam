...
- the compatibility layer in `scene/coords.rs` now exposes signed `ScreenPos` and `project_world_to_screen(...)` for central companion projection plus hero, guide, and vine rendering, while keeping `world_to_screen(...)` as a legacy world-shaped wrapper for debug projection helpers and older bridge paths that still need migration
