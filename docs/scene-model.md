...
- the compatibility layer in `scene/coords.rs` now exposes signed `ScreenPos` and `project_world_to_screen(...)` for active render projection, including central companion projection plus hero, guide, vine, and debug rendering; `resolve_position(...)` remains as the legacy world-shaped bridge for older compatibility paths that still need migration
