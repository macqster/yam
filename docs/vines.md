...
- Vine work must not require changing the active hero appearance. Note this is now a vine-scoped constraint, not a repo-wide art freeze: hero art and its pipeline became the 0.4 main goal on 2026-07-25 and are in scope for deliberate change there (see [`audit.md`](audit.md)'s Hero Track section). The rule survives because it stops vine work from dragging the hero along as a side effect — if the hero changes on its own track, vine work adapts to it rather than driving it.
