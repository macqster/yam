...
def load_scene_config(path: Path) -> SceneConfig:
    ...
    if not path.exists():
        return SceneConfig(
            clock_font_path=Path("v2/render/fonts/go_deco.txt"),
            gif_path=Path("visualizer/assets/source.gif"),
            day_format="%A",
        )

    data = json.loads(path.read_text(encoding="utf-8"))
    clock_font_path = Path(data.get("clock_font_path", "v2/render/fonts/go_deco.txt"))
    ...
    return SceneConfig(
        clock_font_path=clock_font_path,
...
def dump_scene_config(config: SceneConfig, path: Path) -> None:
    ...
    payload = {
        "clock_font_path": str(config.clock_font_path),
...
def update_scene_config_value(path: Path, key: str, value: str) -> SceneConfig:
    ...
    elif key == "clock_font_path":
        config.clock_font_path = Path(value)
    elif key == "day_format":
